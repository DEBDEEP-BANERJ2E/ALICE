...
class AliceEnvironment(Environment):
    ...
    Example:
        ...
        >>> obs = env.reset()
        >>> print(obs.echoed_message)  # "Alice environment ready!"
        >>>
        >>> obs = env.step(AliceAction(message="Hello"))
    ...
    def reset(self) -> AliceObservation:
        ...
        Returns:
            AliceObservation with a ready message
        """
        self._state = State(episode_id=str(uuid4()), step_count=0)
        self._reset_count += 1

        return AliceObservation(
            echoed_message="Alice environment ready!",
            message_length=0,
            done=False,
            reward=0.0,
        )
    ...
    def step(self, action: AliceAction) -> AliceObservation:  # type: ignore[override]
        ...
        Args:
            action: AliceAction containing the message to echo

        Returns:
            AliceObservation with the echoed message and its length
        """
